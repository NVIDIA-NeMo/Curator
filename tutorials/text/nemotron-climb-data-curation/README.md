...
For reference, the tutorial was tested following these steps:

- `docker run --runtime nvidia --gpus all --ipc=host --shm-size=16g -it --rm -v /path/to/data:/path/to/data -v /path/to/Curator:/path/to/Curator -e PIP_CONSTRAINT= nvcr.io/nvidia/pytorch:26.03-py3` (use the most recent version)
- `git clone https://github.com/NVIDIA/Megatron-LM.git`
- `cd Megatron-LM`
- `pip install "setuptools<80.0.0,>=77.0.0" "packaging>=24.2"`
- `pip install --no-build-isolation .[mlm]`
- `bash /path/to/Curator/tutorials/text/nemotron-climb-data-curation/6_train.sh ...`

A proxy model can be trained for each of the `--num-mixtures 64` data ratios generated by step 5. The above snippet trains a single proxy model using the first mixture. The script looks for 4 inputs in order:
