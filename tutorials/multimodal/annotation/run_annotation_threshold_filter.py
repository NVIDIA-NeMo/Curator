...
def build_stage(args: argparse.Namespace) -> InterleavedAnnotationThresholdFilterStage:
    return InterleavedAnnotationThresholdFilterStage(
        blur_min_sharpness=None if args.no_blur else args.blur_min_sharpness,
        blur_score_column="sharpness",
        clip_min_score=None if args.no_clip else args.clip_min_score,
        clip_scores_column="clip_scores",
        qrcode_max_area_ratio=None if args.no_qrcode else args.qrcode_max_ratio,
        qrcode_ratio_column="qr_area_ratio",
        image_text_min_ratio=None if args.no_image_text else args.image_text_min_ratio,
        image_text_max_ratio=None if args.no_image_text else args.image_text_max_ratio,
        image_text_image_num_column="image_num",
        image_text_word_num_column="text_word_num",
    )
...
def main() -> None:
    ...
    for pf in parquet_files:
        ...
        batch.data["_row_id"] = range(len(batch.data))
        n_in = len(batch.data)
        ...
        dropped_ids = set(batch.data["_row_id"]) - set(df_out["_row_id"])
        if dropped_ids:
            dropped = batch.data[batch.data["_row_id"].isin(dropped_ids)].drop(columns=["_row_id"])
            print(f"  Dropped rows:\n{dropped[['sample_id', 'position', 'modality']]}\n")
            import pdb
