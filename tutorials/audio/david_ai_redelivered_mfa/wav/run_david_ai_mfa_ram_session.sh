#!/bin/bash
# Strict on-the-fly David AI mixed-WAV E2E pipeline.
#
...
# manifest-mask speaker WAVs -> mixed 16 kHz PCM WAV -> ordinary and FastMSS
# TextGrids. Masked per-speaker WAVs and their RTTMs are persisted.
# No persisted manifests, shared lexicon, or partial output cache is read. A
# validated session.done flag skips completed sessions and is written only after
# every required output passes validation.
...
WORK_DIR="${WORK_DIR:-$SCRIPT_DIR/workdir_e2e_wav}"
AUDIO_MASKED_DIR="${AUDIO_MASKED_DIR:-$WORK_DIR/audio_16k_masked}"
...
mkdir -p "$LOG_DIR" "$AUDIO_MASKED_DIR" "$TEXTGRID_DIR" "$AUDIO_MIXED_DIR" "$RAM_DIR"
...
CMD=(
    ...
    --data-root "$DATA_ROOT"
    --work-dir "$WORK_DIR"
    --audio-masked-dir "$AUDIO_MASKED_DIR"
...
echo "Audio output=masked per-speaker + session mixed mono 16 kHz PCM WAV"
