...
def setup_mfa_worker_root(
    ...
    *,
    ...
    Returns (mfa_root, local_dict_path, acoustic_model_arg) for ``mfa align``.
    """
    import zipfile
    ...
    if marker.is_file() and not force:
        ...
        return mfa_root, local_dict, acoustic_arg
...
def resolve_mfa_g2p_model(mfa_g2p: str) -> Path:
    ...
    for candidate in (
        mfa_models_root() / "pretrained_models" / "g2p" / f"{mfa_g2p}.zip",
        mfa_models_root() / "pretrained_models" / "g2p" / mfa_g2p,
    ):
        if candidate.is_file() or candidate.is_dir():
            return candidate.resolve()
    raise FileNotFoundError(
        f"MFA G2P model not found for {mfa_g2p!r} under "
        f"{mfa_models_root() / 'pretrained_models' / 'g2p'}"
    )
...
def encode_audio_mono_f32_to_opus(
    ...
    *,
    ...
    try:
        result = subprocess.run(cmd, input=audio.tobytes(), capture_output=True, check=False)
    except OSError as exc:
