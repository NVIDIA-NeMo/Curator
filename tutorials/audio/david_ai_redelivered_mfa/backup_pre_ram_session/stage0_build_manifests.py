...
_DIGIT_DECADE_RE = re.compile(r"(?<![\w])'?(\d{1,4})s\b", re.IGNORECASE)
...
def preprocess_spoken_numbers(text: str, *, num2words_lang: str) -> str:
    ...
    def _decade(match: re.Match[str]) -> str:
        return f"{verbalize_digit_string(match.group(1), num2words_lang=lang)} s"
    ...
    def _general(match: re.Match[str]) -> str:
        return verbalize_digit_string(match.group(0), num2words_lang=lang)
...
def normalize_text(
    ...
    *,
    ...
    try:
        prepared = strip_digit_grouping_commas(text)
