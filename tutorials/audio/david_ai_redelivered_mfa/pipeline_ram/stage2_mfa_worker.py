...
def main() -> int:
    ...
    worker_mfa_root, local_dict, acoustic_arg, g2p_arg = setup_mfa_worker_root(
        worker_dir,
        mfa_dict=mfa_dict,
        mfa_acoustic=args.mfa_acoustic,
        mfa_g2p=args.mfa_g2p or None,
    )
    ...
    for session_id in session_ids:
        if not args.force and is_session_alignment_done(
            session_id,
    ...
    result_path.write_text(
        json.dumps({"worker_id": args.worker_id, "ok": ok, "fail": fail, "skipped": skipped}),
        encoding="utf-8",
    )
    logger.info("worker %02d done: ok=%d fail=%d skipped=%d", args.worker_id, ok, fail, skipped)
