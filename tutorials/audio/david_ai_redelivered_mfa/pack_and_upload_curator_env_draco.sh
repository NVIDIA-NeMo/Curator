...
rsync -avP "$SCRIPT_DIR/install_curator_pain_1_draco.sh" \
    "${DRACO_USER}@${DRACO_HOST}:${CLUSTER_BASE}/Curator/tutorials/audio/david_ai_redelivered_mfa/"

log "On draco run:"
log "  ssh ${DRACO_USER}@${DRACO_HOST}"
log "  CLUSTER_BASE=$CLUSTER_BASE bash $CLUSTER_BASE/Curator/tutorials/audio/david_ai_redelivered_mfa/install_curator_pain_1_draco.sh"
