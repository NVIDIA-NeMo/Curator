...
def test_cluster_launchers_are_executable_and_do_not_copy_data():
...
def test_multinode_launcher_uses_explicit_slurm_array_sharding():
...
def test_both_pipeline_variants_are_supported():
    submit = SUBMIT.read_text(encoding="utf-8")
    node = NODE.read_text(encoding="utf-8")
...
def test_mfa_scratch_and_model_copies_are_isolated_per_shard_and_worker():
    node = NODE.read_text(encoding="utf-8")
