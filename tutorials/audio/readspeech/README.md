...
- **Output**: Integer label (1 = full_band, 0 = narrow_band)
...
| VAD (Silero)           | No            | `cpus=1.0, gpus=0.0` | —                   | CPU by default; opt-in GPU via `Resources(gpus=0.3)` |
| Band Filter            | No            | `cpus=4.0`           | —                    | scikit-learn Random Forest, always CPU               |
| UTMOS                  | No            | `cpus=1.0, gpus=0.5` | ~1.5 GB             | PyTorch; uses GPU if available, falls back to CPU    |
| SIGMOS                 | No            | `cpus=1.0, gpus=0.5` | ~1.3 GB             | ONNX model (P.804); uses GPU if available, falls back to CPU (`force_cpu=True`) |
| Speaker Separation     | No            | `cpus=1.0, gpus=1.0` | ~1.5 GB             | Sortformer NeMo model; supports CPU via `Resources(gpus=0)` but significantly slower |

> **Measured on RTX 4090 (24 GB)** with 10 samples, all filters enabled. Peak total VRAM: **~1.5 GB**. Stages run as separate Ray actors in batch mode, so VRAM is not cumulative — each stage loads its model, processes, then releases GPU memory. Spikes are brief (1–3 seconds per stage).
...
Measured on a single NVIDIA GeForce RTX 4090 (24 GB VRAM), DNS Challenge Read Speech dataset, all filters enabled (UTMOS + VAD + SIGMOS + Band + Speaker Separation):

| Configuration                  | Samples | Approximate Wall-Clock Time |
|--------------------------------|---------|-----------------------------|
| Validate setup (`--max-samples 10`, all filters) | 10      | ~2–3 min |
| Default (`--max-samples 5000`) | 5,000   | ~15–25 min                  |
| Full dataset (`--max-samples -1`) | 14,279  | ~45–75 min                  |
| UTMOS-only (no VAD)            | 5,000   | ~10–15 min                  |
...
| All filters (UTMOS + SIGMOS + VAD + Band + Speaker Sep) | ~50–65%          | Most aggressive; yields highest quality    |
...
| `ModuleNotFoundError: nemo_toolkit` | Reinstall with `uv sync --extra audio_cuda12` |
...
4. **Expected stage durations** (5000 samples, RTX 4090):
