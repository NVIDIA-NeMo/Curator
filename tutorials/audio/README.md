...
| **[Qwen-Omni In-Process ASR](qwen_omni_inprocess/)** | Run the Granary v2 first inference stage with Qwen3-Omni and sharded manifest outputs. Uses the generic `ASRStage` + swappable `ASRAdapter` split and duration-bucketed batching, so models swap with one `adapter_target:` line. | `main.py`, `qwen_omni_inprocess.yaml` |
