...
| **[Qwen-Omni In-Process ASR](qwen_omni_inprocess/)** | Run in-process ASR with Qwen3-Omni and sharded manifest outputs. Uses `ASRStage` + swappable `ASRAdapter`, generic `BucketedInferenceStage` / `BatchPolicy` bucketing, and `perf_summary.json` with stage identity and per-GPU scheduling breakdown (all stages metered via `BaseStageAdapter`; single writer serializes to `perf_summary.json`). | `main.py`, `qwen_omni_inprocess.yaml` |
