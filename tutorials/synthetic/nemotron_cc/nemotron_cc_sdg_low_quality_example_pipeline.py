...
def create_preprocessing_pipeline(args: argparse.Namespace) -> Pipeline:
    ...
    # Define pipeline
    pipeline = Pipeline(name="nemotroncc_preprocessing", description="Preprocess the input data for the Nemotron-CC pipeline")
    ...
    pipeline.add_stage(
        TokenCountFilter(
            ...
            input_field="text",
            output_field="document_token_count",
        ),
    )
    ...
    pipeline.add_stage(
        Filter(
            filter_fn=lambda x: int(x) <= args.max_token_count,
...
def create_wikipedia_postprocessing_pipeline((args: argparse.Namespace) -> Pipeline:
    """Create image curation pipeline with file partitioning, image reading, embedding, aesthetic scoring, and NSFW detection stages."""
