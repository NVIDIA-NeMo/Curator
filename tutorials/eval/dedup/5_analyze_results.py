...
"""
Summarize the fuzzy-dedup-eval LLM judge output (step 5): bucket the judge's
...
def summarize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for pair_type, group in df.groupby("pair_type"):
        total = len(group)
        relation_counts = group["relation_type"].value_counts().to_dict()
        ...
        rows.append(
            {
                ...
                **{f"relation_{k}": v for k, v in relation_counts.items()},
                disagreement_label: group["is_disagreement"].sum() / total if total else float("nan"),
            }
        )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--judge-output-path",
        required=True,
        help="Directory of JSONL or Parquet part files written by 4_run_llm_judge.py (step 4).",
    )
    parser.add_argument(
