...
# Opt-in installer that adds software h264/hevc/av1 decoder support to the
# Curator container by recompiling FFmpeg from source with those decoders
# added to the existing allowlist.
#
# The default Curator image deliberately ships an NVDEC-only ffmpeg so that
# every h264/hevc/av1 decode goes through the GPU. That breaks ffprobe-based
# metadata extraction in CPU-only Ray actors (VideoReader, ClipWriter), which
# is why this opt-in exists.
#
# Run inside a running container, e.g.:
#   docker exec <container> bash /opt/Curator/docker/common/install_h264_support.sh
#
# Behaviour:
#   - Recompiles FFmpeg from upstream git, same tag/branch as install_ffmpeg.sh,
...
#   - Takes ~5-10 min.
#
# Keep FFMPEG_BRANCH and NVCODEC_VERSION in sync with docker/common/install_ffmpeg.sh.
...
usage() {
    cat <<'EOF'
Usage: install_h264_support.sh [--with-libopenh264] [--FFMPEG_BRANCH=<tag>]
                               [--NVCODEC_VERSION=<ver>]

Recompiles FFmpeg from source with software h264/hevc/av1 decoders enabled.
...
Options:
  ...
  --FFMPEG_BRANCH=<tag>      FFmpeg upstream tag/branch (default: n8.0.1).
...
for arg in "$@"; do
    case $arg in
        ...
        --FFMPEG_BRANCH=?*)        FFMPEG_BRANCH="${arg#*=}" ;;
...
echo "==> install_h264_support.sh: rebuilding ffmpeg ${FFMPEG_BRANCH}"
...
echo "    NOTE: This expands the container's codec footprint beyond the default"
echo "    NVDEC-only allowlist. License obligations of the resulting binaries"
...
cd /tmp
rm -rf ffmpeg
git clone --depth 1 --branch "${FFMPEG_BRANCH}" --recurse-submodules \
