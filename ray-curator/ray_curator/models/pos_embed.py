...
# --------------------------------------------------------
def get_3d_sincos_pos_embed(embed_dim: int, grid_size: int, t_size: int, cls_token: bool = False) -> np.ndarray:
...
def interpolate_pos_embed(checkpoint_model: dict[str, Any], model: torch.nn.Module, orig_t_size: int = 4, pos_name: str = "vision_encoder.pos_embed") -> None:
    if pos_name in checkpoint_model:
        ...
        if orig_size != new_size:
            ...
            pos_tokens = pos_tokens.reshape(1, new_t_size, new_size, new_size, embedding_size)
