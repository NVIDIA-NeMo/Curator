...
<Note>
For step-by-step migration instructions, refer to the [Migration Guide ](/migration-guide).
...
Each stage specifies its own resource needs in code (see [Pipeline Execution Backends ](/reference-execution-backends) for configuration details). Set required CPU or GPU count, GPU VRAM, and other specs directly. Ray packs tasks optimally (for example, several light jobs on one GPU).
</Accordion>
...
NeMo Curator suggests several strategies (see [Heuristic Filtering ](/text-process-data-filter-heuristic) and [Quality Assessment ](/about-concepts-text-data-processing) for details):
...
Ray clusters can be deployed on any major cloud platform (AWS, GCP, Azure) using standard Ray tools (see [Ray documentation](https://docs.ray.io/en/latest/cluster/getting-started.html)). No custom infrastructure is needed. NVIDIA provides ready-to-use [Docker images ](/reference-infrastructure-container-environments) and up-to-date quickstart guides. For complete deployment details, refer to [Production Deployment Requirements ](/admin-deployment-requirements).
</Accordion>

<Accordion title="Do I need to build custom Docker images?">
Not for most standard uses. Use [provided images ](/reference-infrastructure-container-environments) or extend as needed (for example, to add proprietary or additional filters). Check out the official Docker container releases on the [NGC Catalog](https://catalog.ngc.nvidia.com/orgs/nvidia/containers/nemo-curator).
</Accordion>
...
Yes. NeMo Curator supports multiple data modalities including [text ](/gs-text), [image ](/gs-image), [audio ](/gs-audio), and [video ](/gs-video).
</Accordion>
...
- [Migration Guide ](/migration-guide) for step-by-step migration instructions
