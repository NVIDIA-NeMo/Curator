...
This design has delivered **~5× performance improvements** on CPU-only stages in pipelines that also use GPUs, because CPU stages no longer block GPU resources.
...
model_stage = ModelStage(model_path="path/to/model").with_resources(Resources(gpus=1))
```
...
Not every GPU stage needs an entire GPU. NeMo Curator supports fractional allocation so multiple lightweight stages can share a single GPU:

```python
# 4 workers share one GPU
