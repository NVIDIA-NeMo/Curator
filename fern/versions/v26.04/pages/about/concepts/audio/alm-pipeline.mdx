...
The builder stage tracks why segments are excluded through a `stats` dictionary on each output task. Loss categories include bandwidth below threshold (`lost_bw`), sample rate below threshold (`lost_sr`), speaker count outside range (`lost_spk`), and window duration outside tolerance (`lost_win`). These statistics help diagnose pipeline yield and tune parameters.

## Overlap Filtering

`ALMDataOverlapStage` removes redundant windows that share too much audio content. The stage sorts windows by start time and calculates the overlap ratio between consecutive pairs.
...
| 50 | Remove windows with greater than 50% overlap | Balanced filtering |
| 100 | Keep all windows regardless of overlap | No filtering |

When two windows overlap beyond the threshold, the stage keeps the window whose duration is closest to `target_duration` and discards the other.
...
Each line of the output JSONL manifest contains the original fields plus pipeline results:
