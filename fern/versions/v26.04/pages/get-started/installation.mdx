...
The build takes ~5–10 minutes, replaces `/usr/local/bin/{ffmpeg,ffprobe}` in place, and pins to the same FFmpeg tag as the image build. Script source: [tools/install_h264_support.sh](https://github.com/NVIDIA-NeMo/Curator/blob/main/tools/install_h264_support.sh).
...
For users distributing customized images, edit [`tools/install_ffmpeg.sh`](https://github.com/NVIDIA-NeMo/Curator/blob/main/tools/install_ffmpeg.sh) before building the container:
