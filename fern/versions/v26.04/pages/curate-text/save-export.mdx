...
```bash
output/
├── {hash}.bin    # Packed token IDs (binary)
├── {hash}.idx    # Sequence metadata (lengths, offsets, document boundaries)
├── {hash}.bin
├── {hash}.idx
```
...
- Per-sequence: 4-byte length, 8-byte byte offset, 8-byte document index
