...
| `spill_memory_limit` | int \| "auto" \| None | "auto" | Device memory spill-to-host limit in bytes. `"auto"` sets the limit to 80% of the RMM pool size; `None` disables spilling. |
...
- For large cluster runs, tune `total_nparts`, `rmm_pool_size`, and `spill_memory_limit` explicitly to match your hardware and dataset size
</Accordion>
