...
3. Compares consecutive pairs and calculates the overlap ratio (overlap duration divided by the shorter window duration)
4. When the overlap ratio exceeds the threshold, keeps the window closest to `target_duration`
...
| 50 | Remove windows with greater than 50% overlap | Balanced yield and diversity |
| 100 | Keep all windows regardless of overlap | No filtering, maximum output |
