...
- Input, output, and checkpoint directories on a shared filesystem or mounted at the same path in every container. The checkpoint directory must be on a filesystem that supports memory-mapped I/O (Lustre, GPFS, BeeGFS, or local disk) — not NFS. See [Checkpoint Storage](/reference/infra/resumable-processing#checkpoint-storage).
