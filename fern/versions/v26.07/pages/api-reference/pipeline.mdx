...
def run(
    ...
    Args:
        ...
        checkpoint_path: Optional resumability directory. Completed source
            ...
            when the pipeline runs again with the same path. Requires a Ray
            cluster started before `run()` and every stage marked
            `is_resumable`. In multi-node or distributed runs, this must
            resolve to the same durable location on every worker and node, on
            a filesystem that supports memory-mapped I/O (local disk, Lustre,
            GPFS, BeeGFS — not NFS).
...
Resumability requires a Ray cluster that is already running, so start one before `run()`:
...
Run the same pipeline with the same path after an interruption. Sources recorded as completed are skipped; incomplete sources restart from the source stage. Refer to [Resumable Processing](/reference/infra/resumable-processing) for the full set of prerequisites.
