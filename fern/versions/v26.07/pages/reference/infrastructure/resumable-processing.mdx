...
There is no guard against this in 26.07: an unusable mount stalls the checkpoint actor with no error. Verify the filesystem before pointing a long run at it.
</Warning>
...
- Confirm the expected number of partitions is reprocessed after a restart.
