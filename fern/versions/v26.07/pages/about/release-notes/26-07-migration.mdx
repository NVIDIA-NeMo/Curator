...
- `checkpoint_path` must be a filesystem directory, not an object-store or cloud URI, on a filesystem that supports memory-mapped I/O: local disk, Lustre, GPFS, or BeeGFS. NFS and other remote filesystems are unsupported by LMDB and can stall the checkpoint actor. State is written under `.nemo_curator_metadata`.
