---
title: NeMo Curator Migration FAQ
...
Each stage specifies its own resource needs in code (see [Pipeline Execution Backends](/docs/reference/execution/backends) for configuration details). Set required CPU or GPU count, GPU VRAM, and other specs directly. Ray packs tasks optimally (for example, several light jobs on one GPU).
</Accordion>
...
NeMo Curator suggests several strategies (see [Heuristic Filtering](/docs/text/process/data/filter/heuristic) and [Quality Assessment](/docs/about/concepts/text/data/processing) for details):
...
Yes, by design. Add new stages or modify process functions to integrate custom logic, models, or data preprocessing and postprocessing (see [Key Abstractions](/docs/about/concepts/video/abstractions) for examples). Extend or fork example pipelines to suit new use cases.
</Accordion>
...
Ray clusters can be deployed on any major cloud platform (AWS, GCP, Azure) using standard Ray tools (see [Ray documentation](https://docs.ray.io/en/latest/cluster/getting-started.html)). No custom infrastructure is needed. NVIDIA provides ready-to-use [Docker images](/docs/reference/infrastructure/container/environments) and up-to-date quickstart guides. For complete deployment details, refer to [Production Deployment Requirements](/docs/admin/deployment/requirements).
</Accordion>

<Accordion title="Do I need to build custom Docker images?">
Not for most standard uses. Use [provided images](/docs/reference/infrastructure/container/environments) or extend as needed (for example, to add proprietary or additional filters). Check out the official Docker container releases on the [NGC Catalog](https://catalog.ngc.nvidia.com/orgs/nvidia/containers/nemo-curator).
</Accordion>
...
Yes. NeMo Curator supports multiple data modalities including [text](/docs/get-started/text), [image](/docs/get-started/image), [audio](/docs/get-started/audio), and [video](/docs/get-started/video).
</Accordion>
...
- Migration Guide for step-by-step migration instructions
