---
title: "NeMo Curator Release Notes: 25.09"
...
NeMo Curator now supports comprehensive [video data curation](/curate-video) with distributed processing capabilities:

- **Video splitting**: [Fixed-stride](/curate-video/process-data/clipping) and [scene-change detection (TransNetV2)](/curate-video/process-data/clipping) for clip extraction
- **Semantic deduplication**: [K-means clustering and pairwise similarity](/curate-video/process-data/dedup) for near-duplicate clip removal
- **Content filtering**: [Motion-based filtering](/curate-video/process-data/filtering) and [aesthetic filtering](/curate-video/process-data/filtering) for quality improvement
- **Embedding generation**: InternVideo2 and Cosmos-Embed1 models for clip-level embeddings
- **Enhanced captioning**: [VL-based caption generation with optional LLM-based rewriting](/curate-video/process-data/captions-preview) (Qwen-VL and Qwen-LM supported) for detailed video descriptions
- **Ray-based distributed architecture**: Scalable video processing with [autoscaling support](/about/concepts/video/architecture)

### Audio

New [audio curation capabilities](/curate-audio) for speech data processing:

- **ASR inference**: [Automatic speech recognition](/curate-audio/process-data/asr-inference) using NeMo Framework pretrained models
- **Quality assessment**: [Word Error Rate (WER) and Character Error Rate (CER)](/curate-audio/process-data/quality-assessment) calculation
- **Speech metrics**: [Duration analysis and speech rate metrics](/curate-audio/process-data/audio-analysis) (words/characters per second)
- **Text integration**: Seamless integration with [text curation workflows](/curate-audio/process-data/text-integration) via `AudioToDocumentStage`
...
- **Ray backend migration**: Complete transition from Dask to Ray for distributed [text processing](/curate-text)
- **Improved model-based classifier throughput**: Better overlapping of compute between tokenization and inference through [length-based sequence sorting](/curate-text/process-data/quality-assessment/distributed-classifier) for optimal GPU memory utilization
...
- **Pipeline-based architecture**: Transitioned from legacy `ImageTextPairDataset` to modern [stage-based processing](/curate-images) with `ImageReaderStage`, `ImageEmbeddingStage`, and filter stages
...
- **Modular processing stages**: Separate stages for [embedding generation](/curate-images/process-data/embeddings), [aesthetic filtering](/curate-images/process-data/filters/aesthetic), and [NSFW filtering](/curate-images/process-data/filters/nsfw)
...
Learn more about [image curation](/curate-images).

## Deduplication Improvements

Enhanced deduplication capabilities across all modalities with improved performance and flexibility:

- **Exact and Fuzzy deduplication**: Updated [rapidsmpf-based shuffle backend](/reference/infra/gpu/processing) for more efficient GPU-to-GPU data transfer and better spilling capabilities
- **Semantic deduplication**: Support for deduplicating [text](/curate-text/process-data/deduplication/semdedup) and [video](/curate-video/process-data/dedup) datasets using unified embedding-based workflows
- **New ranking strategies**: Added `RankingStrategy` which allows you to rank elements within cluster centers to decide which point to prioritize during duplicate removal, supporting [metadata-based ranking](/curate-text/process-data/deduplication/semdedup) to prioritize specific datasets or inputs
...
- **Multiple backends**: Support for [Xenna, Ray Actor Pool, and Ray Data execution backends](/reference/infra/execution-backends)
...
- **Modality-specific guides**: Comprehensive documentation for each supported modality ([text](/curate-text), [image](/curate-images), [audio](/curate-audio), [video](/curate-video))
- **API reference**: Complete [API documentation](https://docs.nvidia.com/nemo/curator/latest/py-modindex.html) with type annotations and examples
