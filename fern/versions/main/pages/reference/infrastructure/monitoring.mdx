...
For a completed audio pipeline, `ManifestWriterStage` can also write a
terminal performance summary with row, duration, stage, and throughput
metrics. See [Save & Export Audio Data](/curate-audio/save-export#manifest-writer-with-a-terminal-performance-summary).
That JSON artifact complements the live time-series monitoring described
here; it does not replace Prometheus or Grafana.
