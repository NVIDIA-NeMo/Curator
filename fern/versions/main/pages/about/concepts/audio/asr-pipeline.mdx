...
**Adapter-based NeMo Framework Integration**: Leverages state-of-the-art ASR models
...
- One shared `ASRStage` contract for task I/O, with `NeMoASRAdapter` for model-specific behavior
...
# The requested resource selects GPU (1.0) or CPU (0).
asr_stage = asr_stage.with_(resources=Resources(gpus=1.0))
```

### Stage 3: Transcription Generation

```python
# Pipeline execution downloads the checkpoint once per node, loads one
# worker-local adapter model, and dispatches batches.
...
asr_stage = ASRStage(
    adapter_target="nemo_curator.models.asr.nemo_asr.NeMoASRAdapter",
    audio_filepath_key="audio_filepath",
    model_id="nvidia/stt_en_fastconformer_hybrid_small"  # Smaller model
).with_(
