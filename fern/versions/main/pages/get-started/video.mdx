...
Curator’s video pipelines rely on `FFmpeg` for decoding and encoding. If you plan to encode clips, install `FFmpeg` with NVENC, VP9, software H.264/HEVC/AV1 decode, and `libopenh264` support. The maintained install script bundles these capabilities.

<Tabs>
<Tab title="Debian/Ubuntu (Script)">

Use the maintained script in the repository to build and install `FFmpeg` with the codecs needed by Curator video workflows. The script enables dynamic CUDA/NVIDIA codec support through `--enable-cuda`, `--enable-cuvid`, `--enable-nvdec`, `--enable-nvenc`, and `--enable-ffnvcodec`, plus `--enable-libvpx` and `--enable-libopenh264`.
...
Confirm that `FFmpeg` is on your `PATH` and that the expected video codecs are available:
