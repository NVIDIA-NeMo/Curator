...
Use the maintained script in the repository to build and install a strict-allowlist `FFmpeg` with NVIDIA NVENC/NVDEC, VP9, software H.264/HEVC/AV1 decoders, and the `libopenh264` software H.264 encoder.

- Script source: [tools/install_ffmpeg.sh](https://github.com/NVIDIA-NeMo/Curator/blob/main/tools/install_ffmpeg.sh)

```bash
curl -fsSL https://raw.githubusercontent.com/NVIDIA-NeMo/Curator/main/tools/install_ffmpeg.sh -o install_ffmpeg.sh
...
Confirm that `FFmpeg` is on your `PATH` and that the expected codecs are available:

```bash
curl -fsSL https://raw.githubusercontent.com/NVIDIA-NeMo/Curator/main/tools/install_ffmpeg.sh -o install_ffmpeg.sh
bash install_ffmpeg.sh --check
```

If codecs are missing, reinstall `FFmpeg` with the required options or use the Debian/Ubuntu script above.
...
<Warning>
The FFmpeg install script links Cisco OpenH264. You are responsible for any license obligations imposed by the resulting binaries.
</Warning>

---

### Install Lynx (Required for Math HTML Extraction)

Curator's math HTML extraction uses the `lynx` command-line browser to convert HTML content to text.

<Tabs>
<Tab title="Debian/Ubuntu (Script)">

- Script source: [tools/install_lynx.sh](https://github.com/NVIDIA-NeMo/Curator/blob/main/tools/install_lynx.sh)

```bash
curl -fsSL https://raw.githubusercontent.com/NVIDIA-NeMo/Curator/main/tools/install_lynx.sh -o install_lynx.sh
chmod +x install_lynx.sh
sudo bash install_lynx.sh
```
