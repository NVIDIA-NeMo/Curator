...
The supplied proxy configuration is single-node. Adapt the launch command and Megatron parallelism settings before using multiple nodes. The default proxy run exits after approximately 110 minutes, saves every 1,000 iterations, and targets 10,000 iterations.
...
| --- | --- | --- |
| `computed_embeddings/` | Text, generated unique IDs, and embedding vectors | Yes, through clustering |
| `clusters/` and `centroids/` | Documents partitioned by centroid and fitted K-means centroids | Yes, through pruning |
| `pruned_clusters/` | Quality-scored JSONL grouped into retained or merged domains | Yes, through tokenization |
...
| `megatron_exp_*/` | Proxy checkpoints, data cache, and TensorBoard logs | Yes, until evaluation succeeds |
...
The script treats the existence of each top-level output directory as a completed stage. This makes clean restarts inexpensive, but a directory left by an interrupted process is also skipped.
