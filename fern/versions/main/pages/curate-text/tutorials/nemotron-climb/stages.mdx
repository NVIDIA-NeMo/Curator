...
Confirm output files are readable and contain all three selected fields. The stage does not expose an append/resume protocol: if a run leaves an incomplete output directory, move or remove that directory and rerun stage 1. Generated IDs include each batch task ID and can change on a new run, so rerun all dependent stages rather than combining old and new stage 1 outputs.
...
- The exact `--id-field`, `--text-field`, and `--embedding-field` names written by stage 1.
...
- The recipe passes the conservative estimate `--embedding-dim 3072` for Stella embeddings. K-means uses this value to split Parquet input into smaller groups around cuDF limits; a larger estimate creates smaller groups and can reduce memory pressure. Adjust it for the model's embedding width and available GPU memory.
- `--fit-data-fraction` fits centroids on a sample while assigning all documents.
...
- A Hugging Face tokenizer identifier or local path. The default Llama 2 model is gated.
...
Stage 4 domain pairs. At least one `.bin` must exist and the combined `.bin` size must be nonzero.
...
To fine-tune a compatible proxy checkpoint, add the optional fifth positional argument:
...
`7_evaluate.sh` discovers all `n<integer>` work directories under one training-round directory and evaluates their checkpoints on ARC-Easy, HellaSwag, and PIQA.
