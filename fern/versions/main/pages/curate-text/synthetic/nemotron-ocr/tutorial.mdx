...
print(merge_output_shards(output))
```

The CLI merges shards only after a successful pipeline run. For stronger recovery, call `merge_output_shards` in your own outer `finally` after workers stop.
