...
`OCRData` extends `ImageTaskData` and is populated incrementally:
...
| `conversation` | Scoring QA | Alternating user/assistant messages, with the image on the first user turn. |
...
The implementation records four QA tags, but single-occurrence and repeated-text routing produces six user-visible shapes:
...
Generation is seeded from the framework task ID. At most 100 QA pairs are balanced across the four internal families. When five or more OCR items are invalid, text-to-location questions are disabled.
