...
`DataDesignerStage` can be serialized and copied to Ray workers even though the live Data Designer client contains process-local runtime objects that are not serializable. The stage serializes its configuration builder and model providers, omits the live client, and recreates that client inside the destination worker. This happens automatically; application code should construct the stage normally and should not create or replace its internal Data Designer client.
