...
`InferenceServer` serves one or more local models behind an OpenAI-compatible endpoint on your Ray cluster. Choose a typed backend configuration for either Ray Serve or NVIDIA Dynamo; the client-facing endpoint and lifecycle API stay the same.
...
models = [
    ...
    RayServeModelConfig(
        model_identifier="HuggingFaceTB/SmolLM-135M-Instruct",
        model_name="reviewer",
        deployment_config={
...
`kv_events` is valid only with KV routing. With `kv_events=False`, Dynamo uses approximate tree-based tracking. Exact event-backed routing publishes vLLM KV events; it is incompatible with explicitly enabling vLLM's hybrid KV-cache manager.
