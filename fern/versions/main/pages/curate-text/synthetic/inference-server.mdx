...
A Dynamo model's actor has its own **isolated Python virtualenv**, built fresh for that actor and not inherited from the environment that launched the driver process. Inside that actor, Dynamo then launches a **worker subprocess** that runs vLLM. `DynamoVLLMModelConfig.runtime_env` is per-model — only that model's actor sees it. `DynamoServerConfig.subprocess_env` is server-wide — it reaches every configured model's worker plus the frontend. Use the mechanism that matches both what you're changing and who needs it:
...
| Set an env var only one model needs (an engine feature flag, a model-specific cache path) | `runtime_env["env_vars"]` on that model config | Scoped to that model's actor; setting it on `subprocess_env` instead would leak it onto every other model's worker |
