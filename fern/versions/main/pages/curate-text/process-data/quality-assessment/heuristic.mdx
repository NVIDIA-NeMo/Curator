...
| `WordsWithoutAlphabetsFilter` | Limits words without alphabetic characters | `min_words_with_alphabets` | `0.8` |
| `BulletsFilter` | Limits content with many bullet points | `max_bullet_lines_ratio` | `0.9` |
| `BoilerPlateStringFilter` | Detects boilerplate text | `max_boilerplate_string_ratio`, `remove_if_at_top_or_bottom` | `0.4`, `True` |
| `ParenthesesFilter` | Limits content in parentheses | `max_parentheses_ratio` | `0.1` |
...
| Filter | Description | Key Parameters | Default Values |
|--------|-------------|----------------|---------------|
| `PornographicUrlsFilter` | Detects URLs containing the "porn" substring | `None` | Not applicable |
...
| `SubstringFilter` | Filters based on the presence of a specific substring in a position | `substring`, `position` | Required |
...
Refer to [Performance Tuning](#performance-tuning) for executor configuration examples that use Xenna or Ray backends.
</Tab>
...
# Or use Ray for distributed processing (refer to Performance Tuning)
