...
| `inference_server` | Expands into `InferenceServer`. It controls lifecycle behavior such as the startup health-check timeout. |
...
Refer to [Install NeMo Curator](/main/get-started/installation#package-extras) for supported installation methods and CUDA requirements.
...
The Curator repository includes an [`llm-judge-config` authoring skill](https://github.com/NVIDIA-NeMo/Curator/blob/main/nemo_curator/eval/llm_judge/LLM_JUDGE_CONFIG_SKILL.md) that helps a coding agent create or revise the YAML and Jinja files. The skill is authoring guidance: `LLMJudgeWorkflow` does not load or execute the skill file at runtime.
...
- The decision the judge should make and the evidence it can use.
...
`inference_server` is also optional and is passed to `InferenceServer`. Increase `health_check_timeout_s` when large model weights need longer than the default startup window. Refer to [Inference Server](/main/curate-text/synthetic/inference-server) for all supported fields and disaggregated-serving examples.
...
| --- | --- | --- |
| Construction fails before model startup | The YAML root is not a mapping, or a filter references an unknown judge, score, or operator. | Correct the configuration reference named in the exception. |
| The inference server times out during startup | The model path is invalid, model loading exceeds `health_check_timeout_s`, or the requested GPU topology is unavailable. | Verify `model`, check the server log, and adjust the timeout or GPU settings. |
| A worker reports a GPU out-of-memory error | The replicas, tensor parallelism, sequence length, or batch capacity exceed the allocation. | Reduce `num_replicas`, `max_model_len`, or `max_num_seqs`. Increase `tensor_parallel_size` when one replica must span more GPUs. |
| Output contains fewer records than input | A configured filter removed rows, or a model response did not satisfy the structured output contract. | Retry a small sample with `with_trace: last_message`, then inspect completion length and rubric output. |
| An output JSONL part is empty | An upstream filter or structured-output rejection removed every row in that partition. | Skip empty files during analysis and compare aggregate input and output counts. |

For distributed execution patterns, refer to [Multi-Node Ray on Slurm](/main/admin/deployment/slurm-multi-node-ray) and [SLURM Job Arrays](/main/admin/deployment/slurm-arrays). Refer to the [LLM judge source directory](https://github.com/NVIDIA-NeMo/Curator/tree/main/nemo_curator/eval/llm_judge) for the implementation. The [LLM judge tutorial source](https://github.com/NVIDIA-NeMo/Curator/tree/main/tutorials/eval/llm_judge) contains the maintained examples.
