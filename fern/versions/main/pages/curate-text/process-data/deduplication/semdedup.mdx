...
1. **Fit:** Each actor samples `round(fit_data_fraction * number_of_actor_files)` complete files, with a minimum of one. A bounded Parquet reader fills one exactly sized fit array while retaining the sampled rows' metadata.
