...
`fit_data_fraction` controls how much input data is used to learn the centroids. All input rows
still receive cluster assignments, regardless of the fit fraction.

For the lower-level workflow and stage, `fit_data_fraction=None` automatically selects a fit sample
based on available GPU memory for Parquet input. JSONL input fits all files by default. Set a fraction
in `(0, 1]` to choose the sample size explicitly, or `1.0` to fit all files.

Sampling selects complete files, so the actual fraction of rows can differ when file sizes vary.
Each GPU actor samples at least one file. Choose enough representative data to fit `n_clusters`
centroids, and use `random_state` or `kmeans_random_state` for repeatable selection.

Parquet is recommended for large datasets. It releases the fitting data before prediction and
processes the full input in smaller groups. Prediction overlaps reads and writes to improve
throughput while preserving the centroid-partitioned output used by pairwise deduplication.
`KMeansStage.predict_write_workers` limits concurrency per GPU (default: `4`); set it to `1` to
compare serial processing when investigating throughput or concurrency problems.

If fitting runs out of memory, reduce `fit_data_fraction`. If prediction runs out of memory,
reducing the fit fraction will not help: prediction still processes every row. Check the available
GPU memory and input file sizes; an individual file may need to be split into smaller files.
JSONL with an explicit fit fraction also rereads the full input for prediction, but does not gain
Parquet's bounded prediction reads.

When comparing benchmark runs, use the full KMeans elapsed time or `kmeans_second_pass_time_s`
for reading, predicting, and writing after fitting. Individual read, prediction, and write activity
can overlap, so adding those timings overstates elapsed time.
