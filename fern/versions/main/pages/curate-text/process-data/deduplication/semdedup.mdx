...
prediction and writing, so it does not compete with their temporary allocations. Prediction memory
sizing is an estimate: a single oversized file or underestimated metadata can still exhaust memory
during reading, prediction, or writing.
An explicit JSONL fit fraction also rereads the input, but loads all files together for prediction.

The Parquet prediction pass uses up to four threads per GPU, with read groups sized from the
available memory, embedding-element counts, and retained-column sizes. `KMeansStage.predict_write_workers`
sets the maximum; use `1` for serial prediction and writing. Each thread has its own predictor and
centroid-partitioned writer. Local output reuses files across frames, creates each centroid directory
only when first needed by a writer, and rotates files before a partition's embedding column reaches
cuDF's element limit. Storage options and additional Parquet
write options continue through the general writer path. Ray workers enable `CUDF_PER_THREAD_STREAM=1`
before importing cuDF and size the shared KvikIO I/O pool to four threads per prediction worker
(`KVIKIO_NTHREADS=16` with the default four workers). An explicit `KVIKIO_NTHREADS` environment value
takes precedence. Set these environment variables before importing cuDF when calling the processing
stage directly. Curator's `KVIKIO_AUTO_DIRECT_IO_WRITE` default is unchanged.

For two-pass benchmarks, compare `kmeans_fit_read_time_s`, `kmeans_fit_time_s`, and
`kmeans_second_pass_time_s`. The second-pass metric measures elapsed time from reading the first
prediction group through closing the final output file, including reads, prediction, and writes.
These are mean per-actor wall times; `kmeans_time` measures the full stage, including actor setup
and the slowest actor. The legacy read/predict/write activity timings remain available for diagnosis,
but overlap during concurrent execution and must not be summed as exclusive phase times.
