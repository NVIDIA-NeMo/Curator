...
Parquet is recommended for large semantic-deduplication workloads. Its footer metadata enables
dtype-aware memory sizing and reads bounded below cuDF's column-size limit. After fitting, Parquet
...
- **Parquet:** Exact footer counts group reads below cuDF's column-size limit and size one contiguous
  float32 fit array. The sampled frames are released during the first pass; the second pass rereads all
  files to predict and write every row.
