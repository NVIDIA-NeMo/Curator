...
| `kmeans_fit_data_fraction` | float \| None | `None` | Fraction of whole files used to fit K-means. With `None`, Parquet automatically selects complete files within the live GPU-memory budget, while JSONL fits all input files. |
...
The behavior of `fit_data_fraction=None` depends on the input format:
