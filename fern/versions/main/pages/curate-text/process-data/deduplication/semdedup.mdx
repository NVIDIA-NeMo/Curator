...
| `pairwise_compute_dtype` | str | `"auto"` | Pairwise multiplication precision: `"auto"`, `"float16"`, or `"float32"` |
| `pairwise_batch_size` | int \| str | `1024` | Positive batch size for similarity computation, or `"auto"` for memory-derived sizing |
| `pairwise_profile` | bool | `False` | Record synchronized read, prepare, similarity, output, and write timings |
...
Pairwise comparison keeps a reusable `N x B` similarity workspace, where `N` is the cluster size and `B` is `pairwise_batch_size`. Set the batch size to `"auto"` to derive a bounded width from currently free GPU memory. Only earlier-ranked rows are valid neighbors; equal similarity scores deterministically choose the earliest-ranked row. The `cosine_sim_score` output is always float32, independent of multiplication precision.

Storage decoding and multiplication precision are separate controls. The lower-level Pairwise stage accepts `input_embedding_dtype`: `"float16"` expects embeddings stored as uint16 FP16 bit patterns, while `"float32"` expects native float32 list leaves. `"auto"` detects these representations and also normalizes legacy float64 list leaves to float32. The workflows expose `pairwise_compute_dtype`; `"auto"` retains the decoded precision, while `"float16"` or `"float32"` casts once before multiplication.

Pairwise always records the cluster row count and resolved batch size. Enable `pairwise_profile=True` to additionally record synchronized read, prepare, similarity, output, and write timings. Overall stage time is already available as the built-in `process_time` metric. Profiling is opt-in because accurate phase boundaries synchronize GPU work and can measurably affect small clusters.
