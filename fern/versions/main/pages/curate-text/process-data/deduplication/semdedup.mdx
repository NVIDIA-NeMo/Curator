...
KMeans fit, prediction, and distance calculations remain FP32. By default, KMeans casts its normalized embedding output to FP16 and stores the bit patterns in uint16 list leaves because cuDF does not support numeric FP16 columns. Set `kmeans_embedding_output_dtype="float32"` to retain FP32 embeddings in the KMeans output. The uint16 representation does not guarantee half-sized Parquet files because Parquet stores uint16 values using an INT32 physical type.
...
FP16 scores remain FP16 through multiplication and reduction, then are promoted to FP32 because cuDF does not support an FP16 numeric output column. Pairwise records footer scan, read, rank, precision conversion, compute, and write time for each cluster, together with its row count and resolved batch size.
