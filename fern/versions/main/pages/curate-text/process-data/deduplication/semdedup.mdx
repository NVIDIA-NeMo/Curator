...
| `kmeans_fit_data_fraction` | float \| None | `None` | Fraction of whole files used to fit K-means. `None` automatically selects complete files within the live GPU-memory budget. |
...
Set a fraction in `(0, 1]` to choose the number of sampled files explicitly:

1. **Fit:** Each actor samples `round(fit_data_fraction * number_of_actor_files)` complete files, with a minimum of one. Exact footer counts group the files below cuDF's column-size limit and fill one exactly sized fit array while retaining the sampled rows' metadata.
2. **Predict sampled rows:** The actor predicts and writes directly from the fit array, so sampled files are not read again.
3. **Predict remaining rows:** The actor reads each remaining file once in element-bounded groups and writes each result frame.
...
<Note>
All input rows receive cluster assignments; the fraction affects only centroid fitting. Both fitting and prediction use exact Parquet footer element counts to keep each read below cuDF's column-size limit.
</Note>
