...
centroid-partitioned writer. Local output reuses files across frames and rotates them before a
partition's embedding column reaches cuDF's element limit. Storage options and additional Parquet
write options continue through the general writer path. Ray workers enable `CUDF_PER_THREAD_STREAM=1`
before importing cuDF; set this environment variable yourself when calling the processing stage directly.
