...
2. **FastText Language Detection**: A FastText-compatible language identification model, such as [`lid.176.bin`](https://fasttext.cc/docs/en/language-identification.html) or [GlotLID](https://huggingface.co/cis-lmu/glotlid), analyzes the preprocessed text and returns:
   ...
   - An uppercase language code (for example, `EN`) or a GlotLID language-script code (for example, `eng_Latn`)
...
- Removing the `__label__` prefix while preserving standard FastText's uppercase output and complete GlotLID labels (for example, `__label__en` becomes `EN` and `__label__eng_Latn` becomes `eng_Latn`)
...
1. **`language` field**: Contains the FastText language identification results as a string representation of a list with two elements (for backend compatibility):
   ...
   - Element 1: The standard uppercase language code or complete GlotLID label without the `__label__` prefix (for example, `EN` or `eng_Latn`)
