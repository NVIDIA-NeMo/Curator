...
| `storage_options` | dict \| None | fsspec options for a remote `download_dir`, also used to read the WARCs back. Install the protocol's backend (`s3fs` for S3, `gcsfs` for GCS). | None |
