...
For a local `download_dir`, the downloader writes each in-progress file to a
temporary `.tmp` path, then moves it into place after a successful download.
For a remote `download_dir`, it stages the file in the local temporary
directory and uploads it only after the download succeeds. For a local
`download_dir`, or one on S3 or GCS, where an upload creates the object only
once it completes, a cancelled or failed download does not look like a
completed WARC file on the next run. Backends that write in place, such as
SFTP or FTP, can leave a partial file after a cancelled upload; delete it
before you rerun.
...
| `download_dir` | str | Local directory or fsspec URL (for example `s3://bucket/cc`) to store downloaded WARC files. Remote WARCs are staged in the local temporary directory before upload, so `TMPDIR` needs room for one WARC per download worker. | Required |
...
| `storage_options` | dict \| None | fsspec options for a remote `download_dir`, also used to read the WARCs back. Install the protocol's backend (`s3fs` for S3, `gcsfs` for GCS); refer to the fsspec [list of known implementations](https://filesystem-spec.readthedocs.io/en/latest/api.html#other-known-implementations). | None |
