...
The shared `backend="vllm"` value names the model engine; the `main.py` entry
point selects the Dynamo HTTP-serving topology. For local validation without an
inference server, run `inprocess.py`, which also supports `backend="hf"`.
