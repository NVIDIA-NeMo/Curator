...
The in-process path automatically selects Triton attention on Ampere GPUs,
following the Nemotron-Parse model's A100/A10 guidance. It also selects Triton
on Blackwell with vLLM versions before 0.23, avoiding the affected
FlashInfer/TRTLLM path. Ray Serve cannot reliably detect the replica GPU while
building its configuration; pass `attention_backend="TRITON_ATTN"` in
`engine_kwargs` for those affected GPUs. Explicit settings always take
precedence.
