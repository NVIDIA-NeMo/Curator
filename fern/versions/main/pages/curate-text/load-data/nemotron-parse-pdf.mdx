...
For the recommended topology, use a fixed HTTP stage pool of `4 * num_gpus` workers and set `inference_batch_size` to 32 or 64 concurrent page requests per worker. Start with 32 and try 64 when the server still has scheduling headroom. The in-process stage retries transient port collisions when starting vLLM; non-retryable startup failures fail immediately.
...
container includes both binaries; a source environment outside the container
must install them before running the entry point.
...
[`dynamo.py`](https://github.com/NVIDIA-NeMo/Curator/blob/main/tutorials/interleaved/nemotron_parse_pdf/dynamo.py)
...
| `inference_batch_size` | int | `4` | Pages per HF pass or concurrent page requests per HTTP worker. Use 32 or 64 with an inference server. |
...
| `num_request_errors` | HTTP | Page requests that still failed after client retries. |
...
Use the quality signals alongside throughput. A high `num_output_length_truncated` value means outputs are reaching the configured `max_tokens` limit (8,192 by default) and warrants inspection of those pages; `num_empty_outputs`, `num_request_errors`, and `num_skipped_pages` identify model-output, request, and image failures that raw pages-per-second figures can hide.
...
3. **HTTP requests:** `NemotronParseHTTPClientStage` uses the existing `AsyncOpenAIClient` exponential-backoff retry behavior. Configure the retry count with `inference_server_max_retries`; requests that still fail are counted in `num_request_errors`.
...
A standalone benchmark script ships at `benchmarking/scripts/nemotron_parse_pdf_benchmark.py`. It uses `TaskPerfUtils` to report stage-normalized per-GPU pages, input tokens, and output tokens per second for both in-process and inference-server runs, along with end-to-end throughput. Use a representative manifest and the public tutorial arguments to compare configurations before scaling to your full corpus; the repository's nightly benchmark orchestration is not required.

## Best Practices

- **Use Dynamo serving for production**: keep one model replica per GPU, four HTTP workers per GPU, and 32 or 64 concurrent requests per HTTP worker. Retain in-process vLLM and HF for small validation runs or debugging.
