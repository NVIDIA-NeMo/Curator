...
| --- | --- | --- |
| `audio_filepath` | Required | Local path readable by FFmpeg. A missing or unreadable file fails an audio-loading stage. |
| `audio_item_id` | Optional | Stable, unique identifier within a shared `resampled_audio_dir`; it determines resampled filenames and speaker IDs. If omitted, resampling derives `<stem>_<path-hash>`. Supply it for reproducible, recognizable output. Reusing an ID can reuse cached audio from another manifest row. |
...
The TTS configuration targets 5–20 second, single-speaker segments. `min_duration: 5` is a threshold used before pause, bandwidth, or punctuation-boundary decisions; short terminal groups can still remain. `full_utterance_ratio: 1.0` prefers complete punctuation-bounded utterances.
...
| `stages.9.min_duration` | `5` | Threshold in seconds used before pause, bandwidth, or punctuation-boundary decisions; it is not a guaranteed output minimum. |
...
| `metrics.start_cer.cer`, `metrics.end_cer.cer` | CER at the first and last `edge_length` characters (default `12`) of the normalized hypothesis and reference text. |
...
- Resampled audio is reused when the destination already exists. Keep supplied `audio_item_id` values unique within a shared `resampled_audio_dir`; otherwise a row can reuse another row's cached audio. Remove stale files from `resampled_audio_dir` after changing a source for an existing ID, sample rate, channel count, or format.
