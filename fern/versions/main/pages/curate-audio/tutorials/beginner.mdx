...
- `ASRStage`: Runs ASR inference with NeMo models
...
def create_audio_pipeline(args):
    ...
    pipeline.add_stage(
        ASRStage(
            ...
            model_id=args.model_name,  # e.g., "nvidia/stt_hy_fastconformer_hybrid_large_pc"
...
2. **ASRStage**: Loads NeMo ASR model and generates transcriptions (requires GPU)
...
- `wer_pct`: Word Error Rate percentage (0.0 = perfect match)
...
cat /data/fleurs_output/result/*.jsonl | jq -r '.wer_pct' | awk '{sum+=$1; count+=1} END {print "Average WER:", sum/count "%"}'
```
...
print(f"Average WER: {df['wer_pct'].mean():.2f}%")
print(f"Average duration: {df['duration'].mean():.2f}s")
print(f"WER range: {df['wer_pct'].min():.2f}% - {df['wer_pct'].max():.2f}%")
```
...
- **Scale across GPUs** by running more workers with `Resources(gpus=1.0)` per NeMo ASR worker
- **Process subset of data** by using `split="dev"` (smaller than "train")
- **Skip ASR inference** if you already have both predicted and target transcriptions. (remove ASRStage)
