...
wer_stage = GetPairwiseWerStage(
    ...
    wer_key="wer_pct",         # Output field for WER percentage
)
...
- `pred_text_key`: Field name containing ASR predictions (from `ASRStage` or similar)
- `wer_key`: Field name to store calculated WER percentages (default: `"wer_pct"`)
