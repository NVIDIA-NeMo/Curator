...
`ComputeWERStage` calculates speech rates together with WER and CER. For top-level records, it requires a positive numeric `duration`. If the manifest does not already include that field, add `GetAudioDurationStage` first. Then configure the hypothesis and reference fields used by your manifest:
