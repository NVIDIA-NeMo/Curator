...
`ComputeWERStage` calculates speech rates together with WER and CER. Configure the hypothesis and reference fields used by your manifest:
...
# Adds metrics.word_rate and metrics.char_rate to each entry or segment.
```
...
<Note>
`PreserveByValueStage` reads top-level fields. Before using the following filters, project `metrics.word_rate` and `metrics.char_rate` into top-level `word_rate` and `char_rate` fields with a small custom stage, or provide those fields in the input manifest.
</Note>
