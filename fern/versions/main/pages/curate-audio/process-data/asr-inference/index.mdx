...
The generic `ASRStage`, configured with `NeMoASRAdapter`, processes
`AudioTask` objects by:

1. **Input Validation**: Verifies required attributes and data structure
2. **Model Loading**: Delegates NeMo checkpoint download and worker-local loading to the adapter
...
asr_stage = asr_stage.with_(
    resources=Resources(
        gpus=1.0,           # One GPU for each NeMo adapter worker
        cpus=4.0            # CPU cores
    )
)
...
Within one executor batch, `ASRStage` loads and normalizes each waveform and
the adapter makes one NeMo transcription call.
</Note>
...
- **Missing or unreadable files**: The row is retained with an empty prediction and `"_skipme": "audio_load_error"`.
