...
The `ASRStage` processes `AudioTask` objects by:
...
armenian_asr = ASRStage(
    adapter_target="nemo_curator.models.asr.nemo_asr.NeMoASRAdapter",
    model_id=language_models["hy_am"],
    audio_filepath_key="audio_filepath",
)
```
...
asr_stage = asr_stage.with_(
    resources=Resources(
        gpus=1.0,           # One GPU for each NeMo ASR worker
        cpus=4.0            # CPU cores
    )
)
...
- **Supported Formats**: Any format readable by the installed `torchaudio` backend.
- **Sample Rates**: Inputs are resampled to `target_sample_rate` (16 kHz by default) before adapter inference.
- **Channels**: Multichannel audio is down-mixed to mono before inference.
...
- **Unreadable files**: By default the stage logs the audio preparation error and marks the task as skipped. Set `fail_on_audio_error=True` to fail the run instead.
