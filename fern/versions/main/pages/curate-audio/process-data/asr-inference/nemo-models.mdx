...
then, from a Curator source checkout, build a bundle on the same GPU architecture
used for inference:
...
`InferenceIndicConformerHybridStage` for an IndicConformer checkpoint. The ASR
stage's `batch_size` is the only user-facing batch control; the runtime
automatically subdivides a stage batch when the serialized engine profile has a
lower maximum. With the default engine chunking mode, both backends split audio
longer than 40 seconds into consecutive chunks and preserve one output transcript
per input.
