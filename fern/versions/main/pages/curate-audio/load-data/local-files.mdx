...
<Warning>
Resolve the local manifest path on the driver before constructing `ManifestReader`. File discovery runs in an executor worker, so a relative path can resolve against a different working directory. Store absolute filesystem paths for audio in the manifest, and make sure those paths resolve identically on every executor. If the source audio is remote, download or otherwise materialize it on that shared or mounted filesystem before running audio stages.
</Warning>
...
create_audio_manifest(
    ...
    manifest_path="/absolute/path/to/local_audio_manifest.jsonl"
)
```
...
create_audio_manifest("/path/to/audio/files", "/absolute/path/to/local_manifest.jsonl")
process_local_audio_manifest("/absolute/path/to/local_manifest.jsonl", "/output/processed_audio")
```
...
create_manifest_with_transcripts(
    ...
    manifest_path="/absolute/path/to/paired_manifest.jsonl"
)
```
...
if validate_manifest("/absolute/path/to/local_manifest.jsonl"):
    process_local_audio_manifest("/absolute/path/to/local_manifest.jsonl", "/output")
```
