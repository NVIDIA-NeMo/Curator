...
<Warning>
Resolve the local manifest path on the driver before constructing `ManifestReader`. File discovery runs in an executor worker, so a relative path can resolve against a different working directory. Store absolute audio paths in the manifest, or use shared storage URIs that every worker can access.
</Warning>
