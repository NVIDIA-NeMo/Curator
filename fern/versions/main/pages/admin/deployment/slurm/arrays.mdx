...
- A SLURM cluster with job-array support.
- A NeMo Curator source checkout and environment visible from every compute node.
- Input, output, and checkpoint directories on a shared filesystem or mounted at the same path in every container.
- Source stages that emit deterministic task IDs. NeMo Curator rejects source IDs beginning with `r` because ambiguous IDs cannot be assigned consistently across retries.
...
| `tutorials/slurm/array_pipeline.py` | JSONL or Parquet reader/writer pipeline |
| `tutorials/slurm/retry_array.py` | Finds incomplete logical shards and formats retry submissions |

## Run a JSONL array

Prepare the source checkout and editable environment on a shared filesystem:
...
NeMo Curator attempts to enable source filtering when either a shard index or total shard count is available, or when `NEMO_CURATOR_SLURM_ARRAY_ENABLED` explicitly enables it. A valid configuration requires both the shard index and total shard count; if either is missing, configuration raises an error instead of running with partial sharding.
...
| `NEMO_CURATOR_SLURM_ARRAY_ENABLED` | None; automatic | Set to `0`, `false`, `no`, or `off` to disable filtering explicitly |
...
- Do not expect a `nemo-curator-slurm-cli` command; use the shipped Python and shell helpers.
