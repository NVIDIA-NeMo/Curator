...
```text
logical shard = sha256(task_id) mod total_shards + minimum_shard_index
```
...
NeMo Curator automatically enables source filtering when both a shard index and total shard count are available.
