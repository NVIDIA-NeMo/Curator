...
Curator uses a [4-step pipeline pattern](/about/concepts/text/data-acquisition-concepts) where data flows through stages as tasks. Each step uses a `ProcessingStage` that transforms tasks according to Curator's [pipeline-based architecture](/about/concepts/text/data-loading-concepts).
