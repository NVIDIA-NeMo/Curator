...
DATASETS = {
    ...
    "FINEWEB_MATH": {
        "input": "hf://HuggingFaceFW/fineweb-math", 
        # "input": "/path/to/fineweb/math/sample/*.parquet",  # Local path alternative
...
def run_dataset(name: str, config: dict, base_output_dir: str):
...
def main():
    ...
    for name in targets:
        if name in DATASETS:
            run_dataset(name, DATASETS[name], args.output_base)
        else:
