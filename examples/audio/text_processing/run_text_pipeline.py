...
_PROMPT_DIR = Path(__file__).resolve().parent.parent.parent.parent / "nemo_curator" / "stages" / "audio" / "text_filtering" / "prompts"
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--input_manifest", type=str, required=True,
                    help="Path to JSONL manifest(s) from the ASR pipeline output. "
                         "Can be a directory or a glob pattern.")
    ap.add_argument("--output_dir", type=str, required=True,
                    help="Output directory for processed manifests.")

    ap.add_argument("--enable_itn", action="store_true", default=False,
                    help="Enable ITN stage: spoken→written form, preserves disfluencies. Output key: itn_text")
    ap.add_argument("--enable_itn_no-disfluencies", action="store_true", default=False,
    ...
    ap.add_argument("--itn_output_key", type=str, default="itn_text",
                    help="Output field for ITN result.")
    ap.add_argument("--itn_no_disfluencies_output_key", type=str, default="itn_no-disfluencies_text",
    ...
    ap.add_argument("--itn_prompt_file", type=str, default=None,
                    help="Path to ITN prompt file. Defaults to bundled itn_prompt.md.")
    ap.add_argument("--itn_no_disfluencies_prompt_file", type=str, default=None,
    ...
    ap.add_argument("--num_workers", type=int, default=None,
                    help="Explicit GPU worker count for Xenna.")
...
def main() -> None:
    ...
    if args.enable_itn_no_disfluencies:
        if not args.enable_itn:
            logger.warning("--enable_itn_no-disfluencies requires --enable_itn (needs itn_text as input). Enabling ITN automatically.")
            stages.append(TextLLMStage(
                name="ITNRestoration",
                prompt_file=itn_prompt,
                text_key=args.text_key,
