...
def test_repo_id_resolves_from_local_snapshot_before_network(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch.object(IndicConformerHybridASR, "download_to_cache") as download,
    ):
...
def test_stage_prefetch_resolves_checkpoint_without_loading_model() -> None:
    ...
    with patch.object(IndicConformerHybridASR, "download_to_cache") as resolve:
        stage.setup_on_node()
...
def test_generate_batches_encoder_by_duration_and_restores_input_order() -> None:
    adapter = IndicConformerHybridASR("checkpoint.nemo", decode_mode="ctc", inference_batch_size=2)
    ...
    assert [call.kwargs["input_signal"].shape[0] for call in model.call_args_list] == [2, 1]
...
def test_generate_uses_batched_rnnt_decoder() -> None:
    adapter = IndicConformerHybridASR("checkpoint.nemo", decode_mode="rnnt", inference_batch_size=3)
    adapter._device = torch.device("cpu")
    model = MagicMock()
