...
"""Unit tests for Task id primitives.

Covers:
- Task._set_task_id (always overwrites task_id; the id is the parents' ids
  joined with this task's own segment)
- Task.get_deterministic_id (default None)
- assign_root_task_ids (roots initial tasks at "0")
...
from nemo_curator.pipeline.pipeline import assign_root_task_ids
...
class TestRootTaskIds:
    def test_empty_task_id_is_zero(self) -> None:
        # EmptyTask is the implicit root id every task descends from.
        assert EmptyTask.task_id == "0"
        assert _EmptyTask(dataset_name="d", data=None).task_id == "0"

    def test_assign_root_task_ids_roots_user_tasks_at_zero(self) -> None:
    ...
    def test_assign_root_task_ids_skips_empty_tasks(self) -> None:
...
class TestSetTaskId:
    def test_initial_state(self) -> None:
        t = _new()
        # task_id is empty until _set_task_id runs.
        assert t.task_id == ""

    def test_sets_task_id(self) -> None:
        t = _new()
        t._set_task_id([], 3)
        # task_id is the joined id path itself (no hashing).
        assert t.task_id == "3"

    def test_filters_empty_parent_ids(self) -> None:
        t = _new()
        # Empty strings in parent ids are stripped (so an unassigned parent
        # doesn't contribute a leading/spurious "_" to the id).
        t._set_task_id(["", "5", ""], 3)
    ...
    def test_string_suffix(self) -> None:
        ...
        t = _new()
        t._set_task_id([], "abc123")
        assert t.task_id == "abc123"

    def test_suffix_under_parent_id(self) -> None:
