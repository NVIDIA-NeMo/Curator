...
def test_sortformer_nightly_inherits_eight_gpus(tmp_path: Path, monkeypatch: MonkeyPatch) -> None:
    ...
    assert entry.requirements["source_num_files"]["exact_value"] == 34
    assert entry.requirements["num_input_files"]["exact_value"] == 34
    assert entry.requirements["num_files_processed"]["exact_value"] == 34
    assert entry.requirements["num_files_with_segments"]["exact_value"] == 34
    assert entry.requirements["num_source_files_scored_for_der"]["exact_value"] == 34
    assert entry.requirements["source_audio_duration_hours"]["min_value"] == 18.7
    assert entry.requirements["total_audio_duration_hours"]["min_value"] == 18.7
    assert "diarization_error_rate_percent" not in entry.requirements
