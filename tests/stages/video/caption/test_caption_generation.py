...
class TestCaptionGenerationStage:
    ...
    def test_init_default_values(self):
        ...
        assert stage.model_variant == "qwen3"
    ...
    def _create_test_video_task(self) -> VideoTask:
        ...
        window1 = _Window(
            start_frame=0,
            end_frame=10,
            llm_inputs={"qwen3": {"prompt": "test prompt 1", "multi_modal_data": {"video": "test_data_1"}}},
        )
        window2 = _Window(
            start_frame=10,
            end_frame=20,
            llm_inputs={"qwen3": {"prompt": "test prompt 2", "multi_modal_data": {"video": "test_data_2"}}},
        )
        ...
        window3 = _Window(
            start_frame=0,
            end_frame=15,
            llm_inputs={"qwen3": {"prompt": "test prompt 3", "multi_modal_data": {"video": "test_data_3"}}},
        )
    ...
    def test_process_successful_generation(self):
        ...
        assert result.data.clips[0].windows[0].caption["qwen3"] == "Caption 1"
        assert result.data.clips[0].windows[1].caption["qwen3"] == "Caption 2"
        assert result.data.clips[1].windows[0].caption["qwen3"] == "Caption 3"

        # Verify cleanup
        for clip in result.data.clips:
            for window in clip.windows:
                assert "qwen3" not in window.llm_inputs
    ...
    def test_process_with_verbose_logging(self):
        ...
        window = _Window(
            start_frame=0,
            end_frame=5,
            llm_inputs={"qwen3": {"prompt": "test", "multi_modal_data": {"video": "data"}}},
        )
        clip.windows = [window]
        video.clips = [clip]
    ...
    def test_process_window_without_input(self, mock_logger: Mock):
        ...
        mock_logger.error.assert_called_once_with(f"Clip {clip.uuid} window 0 has no prepared inputs for qwen3.")
        assert clip.errors["window-0"] == "no_qwen3_input"
    ...
    def test_assign_captions(self):
        ...
        assert video.clips[0].windows[0].caption["qwen3"] == "Caption for clip1 window1"
        assert video.clips[0].windows[1].caption["qwen3"] == "Caption for clip1 window2"
        assert video.clips[1].windows[0].caption["qwen3"] == "Caption for clip2 window1"
...
class TestQwenCaptionPipelineIntegration:
    ...
    def run_pipeline(
        ...
        request.cls.raw_inputs = [w.llm_inputs["qwen3"] for w in clip.windows if "qwen3" in w.llm_inputs]
    ...
    def test_generation_stage_returns_captions(self) -> None:
        ...
        for i, window in enumerate(clip.windows):
            caption = window.caption.get("qwen3")
            assert caption is not None, f"Window {i}: caption key 'qwen3' not set"
            assert isinstance(caption, str), f"Window {i}: caption is not a str"
            assert len(caption.strip()) > 0, f"Window {i}: caption is blank"

        for i, window in enumerate(clip.windows):
            assert "qwen3" not in window.llm_inputs, f"Window {i}: qwen_llm_input not cleared after generation"
