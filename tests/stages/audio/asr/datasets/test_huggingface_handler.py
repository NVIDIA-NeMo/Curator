...
def test_huggingface_handler_writes_manifest_without_returning_tasks(tmp_path: Path) -> None:
    ...
    for i, row in enumerate(rows):
        assert row["audio_filepath"] == str(output_dir / "gu" / "train" / "audio" / f"utt_{i}.wav")
        audio_info = sf.info(row["audio_filepath"])
        assert audio_info.samplerate == OUTPUT_SAMPLE_RATE
        assert audio_info.channels == 1
        assert audio_info.subtype == "PCM_16"
        assert row["text"] == "ગુજરાતી વાક્ય"
        assert row["duration"] == pytest.approx(audio_info.frames / OUTPUT_SAMPLE_RATE)
        assert row["lang"] == "gu"
        assert row["split_type"] == "train"
        assert row["source"] == "Kathbath"
        assert row["orig_sample_rate"] == INPUT_SAMPLE_RATE
        assert row["orig_num_channels"] == 2
        assert row["speaker_id"] == f"speaker_{i}"
        assert row["gender"] == "Female"
        assert row["fname"] == f"utt_{i}.wav"
...
def test_huggingface_handler_rejects_unsupported_source_name(tmp_path: Path) -> None:
