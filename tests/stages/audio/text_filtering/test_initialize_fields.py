...
def test_preserve_pipeline_outputs_true_keeps_output_keys() -> None:
    stage = InitializeFieldsStage(
        drop_keys=["pnc_text", "sed_events", "answer"],
        preserve_pipeline_outputs=True,
    )
    task = AudioTask(
        data={
            "text": "t",
            "pnc_text": "existing pnc",
            "sed_events": [{"event": "speech"}],
            "answer": "remove me",
            "num_speakers": 2,
        }
    )
    result = stage.process(task)
    assert result.data["pnc_text"] == "existing pnc"
    assert result.data["sed_events"] == [{"event": "speech"}]
...
def test_preserve_pipeline_outputs_false_drops_output_keys() -> None:
    ...
    assert "sed_events" not in result.data
    assert "answer" not in result.data
