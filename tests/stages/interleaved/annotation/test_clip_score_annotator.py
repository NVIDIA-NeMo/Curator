...
class TestSampleTextPositionsAndTexts:
...
class TestInterleavedCLIPScoreAnnotatorStage:
    ...
    def test_does_not_drop_rows(self, mock_clip_class: MagicMock, mock_download: MagicMock) -> None:
    ...
    @patch("nemo_curator.stages.interleaved.annotation.clip_score_annotator.CLIPImageEmbeddings")
    def test_no_text_image_gets_na(self, mock_clip_class: MagicMock, mock_download: MagicMock) -> None:
        mock_download.return_value = None
        dim = 512
        mock_model = mock_clip_class.return_value
        mock_model.return_value = torch.ones(1, dim) / (dim**0.5)
    ...
    @patch("nemo_curator.stages.interleaved.annotation.clip_score_annotator.CLIPImageEmbeddings")
    def test_stores_dict_per_image_row(self, mock_clip_class: MagicMock, mock_download: MagicMock) -> None:
        mock_download.return_value = None
        dim = 512
        mock_model = mock_clip_class.return_value
        mock_model.return_value = torch.ones(1, dim) / (dim**0.5)
        ...
        assert isinstance(img_row["clip_scores"], dict)
        assert 0 in img_row["clip_scores"]

    @patch(
        "nemo_curator.stages.interleaved.annotation.clip_score_annotator.CLIPImageEmbeddings.download_weights_on_node"
    )
    @patch("nemo_curator.stages.interleaved.annotation.clip_score_annotator.CLIPImageEmbeddings")
    def test_column_name_is_clip_scores(
        self,
