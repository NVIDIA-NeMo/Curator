...
from nemo_curator.stages.audio.common import GlobalSegmentAssemblerStage
...
def test_real_audio_stages_use_main_worker_sizing_contract() -> None:
    executor = XennaExecutor()
    ...
    assembler_spec = executor._build_stage_spec(GlobalSegmentAssemblerStage())
    ...
    assert assembler_spec.num_workers == 1
    assert assembler_spec.num_workers_per_node is None
