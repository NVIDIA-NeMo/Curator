...
<!-- TODO: Test this entire code snippet -->
...
def build_production_pipeline():
    ...
    # 3. Write intermediate data
    writer = JsonlWriter(path="intermediate_data/")
    pipeline.add_stage(writer)
...
fuzzy_workflow = FuzzyDeduplicationWorkflow(
    input_path="intermediate_data/",
    cache_path="cache/",
    output_path="output_ids/",
...
removal_workflow = TextDuplicatesRemovalWorkflow(
    input_path="intermediate_data/",
    ids_to_remove_path="output_ids/FuzzyDuplicateIds",
    output_path="deduplicated_output/",
    input_filetype="jsonl",  # Same as identification workflow
    input_blocksize="1.5GiB",  # Same as identification workflow
    ids_to_remove_duplicate_id_field="_curator_dedup_id",
    id_generator_path="output_ids/fuzzy_id_generator.json",
)
removal_workflow.run()
