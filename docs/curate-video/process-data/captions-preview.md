...
python tutorials/video/getting-started/video_split_clip_example.py \
  ...
  --enhance-captioning-algorithm-model-id Qwen/Qwen2.5-14B-Instruct \
...
2. Optionally enhance captions with a text‑based LLM (Qwen‑LM) to expand and refine descriptions. This stage reads `window.caption["qwen"]` and writes the result under `window.enhanced_caption[model_id]` (e.g. `window.enhanced_caption["Qwen/Qwen2.5-14B-Instruct"]`).

   ```python
   enh = CaptionEnhancementStage(
       model_dir="/models",
       model_variant="qwen",
       model_id="Qwen/Qwen2.5-14B-Instruct",   # optional – uses default if omitted
       prompt_variant="default",
       prompt_text=None,
...
* - `model_variant`
  - {"qwen"}
  - `"qwen"`
  - Vision‑language model family. Only `"qwen"` is currently supported.
* - `model_id`
  - str | None
  - `None`
