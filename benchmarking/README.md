...
For a 4-GPU H100 environment, layer the SKU override after the full-suite config:

```bash
./benchmarking/tools/run.sh \
  --config ./benchmarking/benchmarks.yaml \
  --config ./benchmarking/4xH100.yaml \
  --config ./benchmarking/nightly-data-setup.yaml
```

The 4xH100 override updates resource counts, timeout values, known 4-GPU video
throughput thresholds, and explicit 8-worker settings. Other performance
requirements are inherited from `benchmarks.yaml` until 4xH100-specific
