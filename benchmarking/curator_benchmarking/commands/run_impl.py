...
The public `run` command keeps argument parsing in `run.py` so minimal host
environments can import the CLI without Curator/runtime dependencies installed.
Once execution reaches this module, we are in the environment that is expected
to run benchmarks, so normal top-level imports are preferred over lazy globals.
"""
...
from runner.path_resolver import set_path_mode
...
def run_benchmarks(args: Namespace) -> int:  # noqa: C901, PLR0911, PLR0912, PLR0915
    if args.path_mode:
