...
DatasetResolver: Any = None
Entry: Any = None
GPUStatsRecorder: Any = None
PathResolver: Any = None
Session: Any = None
TaskPerfUtils: Any = None
WorkflowRunResult: Any = None
assert_valid_config_dict: Any = None
call_sink_hook: Any = None
create_or_overwrite_dir: Any = None
dump_env: Any = None
find_result: Any = None
get_gpu_stats: Any = None
get_obj_for_json: Any = None
get_ray_cluster_data: Any = None
initialize_sinks: Any = None
log_gpu_stats: Any = None
logger: Any = None
merge_config_files: Any = None
remove_disabled_blocks: Any = None
resolve_env_vars: Any = None
resolve_viewer_url: Any = None
run_command_with_timeout: Any = None
setup_ray_cluster_and_env: Any = None
teardown_ray_cluster_and_env: Any = None


def _load_runtime_dependencies() -> None:
    """Import dependencies needed only when a benchmark actually runs."""
    if _RUNTIME_DEPENDENCIES_LOADED:
        return

    from loguru import logger as loaded_logger
    from runner.datasets import DatasetResolver as LoadedDatasetResolver
    from runner.entry import Entry as LoadedEntry
    from runner.env_capture import dump_env as loaded_dump_env
    from runner.gpu_stats_recorder import GPUStatsRecorder as LoadedGPUStatsRecorder
    from runner.path_resolver import PathResolver as LoadedPathResolver
    from runner.process import run_command_with_timeout as loaded_run_command_with_timeout
    from runner.ray_cluster import (
        get_ray_cluster_data as loaded_get_ray_cluster_data,
    )
    from runner.ray_cluster import (
        setup_ray_cluster_and_env as loaded_setup_ray_cluster_and_env,
    )
    from runner.ray_cluster import (
        teardown_ray_cluster_and_env as loaded_teardown_ray_cluster_and_env,
    )
    from runner.session import Session as LoadedSession
    from runner.sinks.sink import call_sink_hook as loaded_call_sink_hook
    from runner.sinks.sink import initialize_sinks as loaded_initialize_sinks
    from runner.utils import (
        assert_valid_config_dict as loaded_assert_valid_config_dict,
    )
    from runner.utils import find_result as loaded_find_result
    from runner.utils import get_gpu_stats as loaded_get_gpu_stats
    from runner.utils import get_obj_for_json as loaded_get_obj_for_json
    from runner.utils import log_gpu_stats as loaded_log_gpu_stats
    from runner.utils import merge_config_files as loaded_merge_config_files
    from runner.utils import remove_disabled_blocks as loaded_remove_disabled_blocks
    from runner.utils import resolve_env_vars as loaded_resolve_env_vars
    from runner.viewer_url import resolve_viewer_url as loaded_resolve_viewer_url

    from nemo_curator.pipeline.workflow import WorkflowRunResult as LoadedWorkflowRunResult
    from nemo_curator.tasks.utils import TaskPerfUtils as LoadedTaskPerfUtils
    from nemo_curator.utils.file_utils import create_or_overwrite_dir as loaded_create_or_overwrite_dir

    globals().update(
        DatasetResolver=LoadedDatasetResolver,
        Entry=LoadedEntry,
        GPUStatsRecorder=LoadedGPUStatsRecorder,
        PathResolver=LoadedPathResolver,
        Session=LoadedSession,
        TaskPerfUtils=LoadedTaskPerfUtils,
        WorkflowRunResult=LoadedWorkflowRunResult,
        assert_valid_config_dict=loaded_assert_valid_config_dict,
        call_sink_hook=loaded_call_sink_hook,
        create_or_overwrite_dir=loaded_create_or_overwrite_dir,
        dump_env=loaded_dump_env,
        find_result=loaded_find_result,
        get_gpu_stats=loaded_get_gpu_stats,
        get_obj_for_json=loaded_get_obj_for_json,
        get_ray_cluster_data=loaded_get_ray_cluster_data,
        initialize_sinks=loaded_initialize_sinks,
        log_gpu_stats=loaded_log_gpu_stats,
        logger=loaded_logger,
        merge_config_files=loaded_merge_config_files,
        remove_disabled_blocks=loaded_remove_disabled_blocks,
        resolve_env_vars=loaded_resolve_env_vars,
        resolve_viewer_url=loaded_resolve_viewer_url,
        run_command_with_timeout=loaded_run_command_with_timeout,
        setup_ray_cluster_and_env=loaded_setup_ray_cluster_and_env,
        teardown_ray_cluster_and_env=loaded_teardown_ray_cluster_and_env,
...
def check_requirements_update_results(  # noqa: C901, PLR0912
...
def run_entry(  # noqa: PLR0913
    ...
    # scratch_path : This is for scratch/temp data; it is cleaned up if delete_scratch is True.
...
def main(argv: list[str] | None = None) -> int:  # noqa: C901, PLR0911, PLR0912, PLR0915
    ...
    try:
        ...
        logger.error(f"Invalid configuration: {e}")  # noqa: TRY400
        return 1
    ...
    for entry in session.entries:
        ...
        try:
            ...
            logger.exception("\t\t❌ Entry failed with exception")
            result_data.update(
                {
                    "error": str(e),
