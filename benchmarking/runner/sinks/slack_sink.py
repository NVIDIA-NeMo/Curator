...
class SlackSink(Sink):
    def __init__(self, config: dict[str, Any]):
        ...
        if not self.webhook_url:
            raise ValueError("SlackSink: No webhook URL configured")
    ...
    def _post_style1(self) -> None:
        ...
        for (var, val) in self.env_data.items():
            row = [
                {"type": "rich_text", "elements": [{"type": "rich_text_section", "elements": [{"type": "text", "text": str(var)}]}]},
                {"type": "rich_text", "elements": [{"type": "rich_text_section", "elements": [{"type": "text", "text": str(val)}]}]},
            ]
            rows.append(row)
        ...
        message_text_values["REPORT_JSON_TEXT"] = ",".join([json.dumps(get_obj_for_json(item), indent=2, sort_keys=True) for item in report_data])
    ...
    def _post_style2(self) -> None:
        ...
        rows = []
        rows.append([{"type": "rich_text", "elements": [{"type": "rich_text_section", "elements": [{"type": "text", "text": "Environment", "style": {"bold": True}}]}]},
                    {"type": "rich_text", "elements": [{"type": "rich_text_section", "elements": [{"type": "text", "text": " "}]}]}])
        for (var, val) in self.env_data.items():
        ...
        rows.append([{"type": "rich_text", "elements": [{"type": "rich_text_section", "elements": [{"type": "text", "text": "Results", "style": {"bold": True}}]}]},
                    {"type": "rich_text", "elements": [{"type": "rich_text_section", "elements": [{"type": "text", "text": " "}]}]}])
        for result in self.results:
            data = [
                ("name", result["name"]),
...
if __name__ == "__main__":
    ...
    # Use the first argument as the directory containing result subdirectories.
    if len(sys.argv) < 2:
        print("Usage: python slack_sink.py <webhook_url> <results_root_dir>")
        sys.exit(1)
    webhook_url = sys.argv[1]
    results_root_path = Path(sys.argv[2])

    def collect_results_from_dir(results_root_path: Path):
