...
class GdriveSink(Sink):
    ...
    def process_result(self, result_dict: dict[str, Any], matrix_entry: MatrixEntry) -> None:
        ...
        logger.debug(f"GdriveSink: Collected result for '{result_entry['benchmark_name']}' (success: {result_dict.get('success', False)})")
    ...
    def finalize(self) -> None:
        if self.enabled:
            ...
            logger.debug(f"GdriveSink: Configuration - Max retries: {self.max_retries}, "
            ...
            if self.results:
                ...
                if configs_with_custom:
                    ...
                    for entry in configs_with_custom:
                        config_summary = ", ".join([f"{k}={v}" for k, v in entry["config"].items() if k != "description"])
                        if config_summary:
                            logger.debug(f"GdriveSink: Benchmark '{entry['benchmark_name']}' custom config: {config_summary}")
                else:
    ...
    def _generate_enhanced_filename(self) -> str:
        ...
        filename_parts = [
            self.session_name,
            timestamp,
            f"{successful_count}of{total_count}success"
        ]
    ...
    def _upload_to_gdrive(self, tar_path: Path) -> str:
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if self.results:
                    ...
                    for entry in self.results:
                        ...
                        if entry_folder_id:
                            ...
                            logger.info(f"GdriveSink: Using custom folder ID from benchmark '{entry['benchmark_name']}': {custom_folder_id}")
                            break
                ...
                logger.info(f"GdriveSink: Upload metrics - Duration: {attempt_duration:.1f}s, Speed: {upload_speed_mbps:.2f} MB/s")
                ...
                if attempt == self.max_retries:
                    ...
                    total_duration = time.time() - upload_start_time
                    logger.error(f"GdriveSink: All {self.max_retries} upload attempts failed after {total_duration:.1f}s")
                    raise
