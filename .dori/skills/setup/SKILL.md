...
**Automated**: First get the latest tag, then pull:

```bash
# Get latest version tag
CURATOR_TAG=$(python skills/setup/scripts/get_latest_tag.py)
echo "Latest tag: $CURATOR_TAG"

# Pull the image
docker pull nvcr.io/nvidia/nemo-curator:$CURATOR_TAG
```

Then verify:

```bash
docker run --rm nvcr.io/nvidia/nemo-curator:$CURATOR_TAG python -c "import nemo_curator; print('Ready')"
```

**Done**: Tell user the image is ready and which version was installed.
...
Example output (replace `<TAG>` with actual version like `25.09`):

```
✅ NeMo Curator is ready!

Run command:
  docker run --rm --shm-size=8g -v $(pwd):/workspace -w /workspace nvcr.io/nvidia/nemo-curator:<TAG> python your_script.py

Test it:
  docker run --rm nvcr.io/nvidia/nemo-curator:<TAG> python -c "import nemo_curator; print('Works!')"
```
...
Then run:
```bash
# Without GPU (macOS/Windows) - include --shm-size for Ray
docker run -it --rm --shm-size=8g \
  -v $(pwd):/workspace -w /workspace \
  nvcr.io/nvidia/nemo-curator:$CURATOR_TAG
...
**Important**: Always include `--shm-size` (use 30%+ of available RAM) for Ray performance.
