...
out of individual stages. Backends still own parent-row scheduling and call
``process_batch`` normally. Inside that call, the stage expands parent tasks into
model-input items, optionally applies ``BatchPolicy`` at item level, calls the
adapter, and stitches results back to parent rows.
...
# Model-input item and per-item result types; intentionally unbounded.
ItemT = TypeVar("ItemT")
...
class BucketedInferenceStage(ProcessingStage[X, Y], Generic[X, Y, ItemT, ResultT]):
    ...
    order. Optional scheduler hooks may exist for specialized callers, but the
    default contract is ordinary backend ``process_batch`` dispatch.
    """
    ...
    def build_items(self, tasks: list[X]) -> tuple[list[ItemT], list[int]]:
        ...
        Returns ``(items, parent_of)`` where ``parent_of[i]`` is the index of the
        task that produced ``items[i]`` (a task may fan out to several items or
        none). Reset any per-call accumulators here, as this hook runs first.
        """
    ...
    def assemble(
        ...
        ``items``/``parent_of``/``results`` are index-aligned. Must return
        exactly one output task per input task, in input order.
        """

    def process_batch(self, tasks: list[X]) -> list[Y]:
        # Ray Data passes columns as numpy ndarrays, so use len() not truthiness
        # (``if not tasks`` would raise ValueError).
        if len(tasks) == 0:
            return []
