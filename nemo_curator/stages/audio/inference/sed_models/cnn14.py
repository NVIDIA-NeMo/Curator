...
Only the decision-level variants that produce framewise output are included.
They are based on the public PANNs AudioSet tagging CNN implementation.

Requires ``torchlibrosa`` at runtime when SED is enabled.
"""
...
try:
    ...
    msg = "Install torchlibrosa to use SED models."
    raise RuntimeError(msg) from exc


def init_layer(layer: nn.Module) -> None:
    """Initialize a linear or convolution layer."""
...
def pad_framewise_output(framewise_output: torch.Tensor, frames_num: int) -> torch.Tensor:
    """Pad framewise output to match the input frame count."""
...
class ConvBlock(nn.Module):
    """Two-layer convolutional block used by CNN14."""
...
class AttBlock(nn.Module):
    """Attention block used by the decision-level attention variant."""
...
def _cnn14_encode(
    ...
    for block, pool_size in zip(blocks, pool_sizes, strict=True):
...
class Cnn14DecisionLevelAvg(Cnn14DecisionLevelMax):
