...
def manifest_data(task: AudioTask, drop_manifest_keys: tuple[str, ...] = ("waveform",)) -> dict[str, Any]:
    """Return a JSON-safe manifest row for ``task``.

    Writers should not serialize in-memory waveform tensors or arrays. Besides
    explicit ``drop_manifest_keys``, this also drops array-like values with
    ``shape`` and ``dtype`` attributes so accidental tensors do not poison the
    JSONL writer path.
    """
    data: dict[str, Any] = {}
...
def manifest_lines(tasks: list[AudioTask], drop_manifest_keys: tuple[str, ...] = ("waveform",)) -> list[str]:
    """Serialize ``tasks`` to JSONL lines using the shared audio writer rules."""
    return [json.dumps(manifest_data(task, drop_manifest_keys), ensure_ascii=False) + "\n" for task in tasks]
