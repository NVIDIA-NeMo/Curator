...
class SelectBestPredictionStage(ProcessingStage[AudioTask, AudioTask]):
    ...
    1. **ASR recovery** -- if QwenASR recovered a hallucinated sample
       (``_skip_me`` starts with ``"Recovered"`` and ``asr_text_key`` is
    ...
    def process(self, task: AudioTask) -> AudioTask:
        ...
        if skip_me.startswith("Recovered") and asr_pred:
            ...
            append_note(task.data, f"SelectBest: used ASR prediction", self.notes_key)
            return task

        both_hallucinated = skip_me.startswith("Hallucination") and asr_pred
        if both_hallucinated and primary_pred:
            wer = get_wer(primary_pred, asr_pred)
            ...
            if wer <= (100.0 - self.min_agreement_pct):
                ...
                append_note(task.data, f"Recovered:CrossModelAgreement WER={wer:.1f}%", self.notes_key)
                return task
