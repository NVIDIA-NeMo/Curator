...
class ImageEmbeddingStage(ProcessingStage[ImageBatch, ImageBatch]):
    ...
    def yield_next_batch(self, task: ImageBatch) -> Generator[list[ImageObject], None, None]:
        """Yield model-sized batches from one image task.

        Args:
            task: ImageBatch containing ImageObject instances with pre-loaded image_data

        Yields:
            Generator[dict[str, torch.Tensor]]: A generator of model inputs for the next batch.

        """
        yield from self._yield_image_batches(task.data)

    def _yield_image_batches(self, images: list[ImageObject]) -> Generator[list[ImageObject], None, None]:
        for i in range(0, len(images), self.model_inference_batch_size):
            yield images[i : i + self.model_inference_batch_size]

    def _embed_images(self, images: list[ImageObject]) -> None:
        """Generate embeddings across task boundaries using full model batches."""
        for batch in self._yield_image_batches(images):
            loaded_images = [img_obj.image_data for img_obj in batch]

            with torch.no_grad():
                embeddings = self.model(loaded_images).cpu().numpy()
    ...
    def process(self, task: ImageBatch) -> ImageBatch:
        ...
        self._embed_images(task.data)
        return task

    def process_batch(self, tasks: list[ImageBatch]) -> list[ImageBatch]:
        """Embed images from multiple transport tasks in full inference batches."""
        for task in tasks:
            if not self.validate_input(task):
                msg = f"Task {task!s} failed validation for stage {self}"
                raise ValueError(msg)

        self._embed_images([image for task in tasks for image in task.data])
        return tasks
