...
# ruff: noqa: ANN202, ANN401, C901

"""Hydra entry point for the Granary v2 Qwen-Omni in-process pipeline.

The raw-audio Hydra config declares logical stages only and uses the
``payload_lifecycle`` section to insert waveform materialization/release helper
stages before executor planning. Compute stages stay visible to Ray Data/Xenna,
so heterogeneous GPU stages keep their own resource contracts. Stage entries
may carry ``stage_with``/``with_`` metadata applied after instantiation to set
resource, batch-size, and composite worker specs.
...
def _instantiate_resources(value: Any) -> Resources:
    ...
    if OmegaConf.is_config(value) or isinstance(value, dict):
        ...
        if target is not None and str(target) not in _ALLOWED_RESOURCES_TARGETS:
            msg = f"resources overrides may only target nemo_curator.stages.resources.Resources; got {target!r}"
...
def _instantiate_configured_stages(cfg: DictConfig) -> list[ProcessingStage]:
    ...
    for stage_cfg in stage_entries:
        ...
        if selected:
            ...
            stage.extended_performance_metrics = bool(cfg.get("extended_performance_metrics", True))
            if stage_id:
                stage._curator_stage_id = str(stage_id)
            stages.append(stage)
    ...
    if unknown_run or unknown_skip:
        ...
        raise ValueError(msg)
    if not stages:
        msg = "No stages selected; check enabled/stages_to_run/stages_to_skip"
        raise ValueError(msg)
    return stages


def build_granary_v2_pipeline(cfg: DictConfig) -> Pipeline:
    """Construct the Granary v2 stage chain from the logical Hydra stage list."""
    return Pipeline(
        name="qwen_omni_inference",
        stages=_instantiate_configured_stages(cfg),
...
def _xenna_executor_config(cfg: DictConfig) -> dict[str, Any]:
    ...
    def _get(key: str, default: Any) -> Any:
        return xenna_cfg.get(key, default)
    ...
    return {
        **_executor_observability_config(cfg),
        ...
        "logging_interval": _get("logging_interval", 60),
        "actor_pool_verbosity_level": _get("actor_pool_verbosity_level", "NONE"),
        "monitoring_verbosity_level": _get("monitoring_verbosity_level", "NONE"),
        "autoscaler_verbosity_level": _get("autoscaler_verbosity_level", "NONE"),
        "executor_verbosity_level": _get("executor_verbosity_level", "NONE"),
        "log_worker_allocation_layout": _get("log_worker_allocation_layout", False),
    }


def _executor_observability_config(cfg: DictConfig) -> dict[str, Any]:
    """Opt the Qwen benchmark pipeline into run-level hardware telemetry."""
    return {
        "pipeline_hardware_sampler_enabled": bool(cfg.get("pipeline_hardware_sampler_enabled", True)),
...
def _create_executor(cfg: DictConfig):
    ...
    if backend == "xenna":
        return executor_cls(config=_xenna_executor_config(cfg))
    ...
    return executor_cls(config=_executor_observability_config(cfg))
