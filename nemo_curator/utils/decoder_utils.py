...
class SoftwareCodecMissingError(RuntimeError):
    ...
    a software decoder for the input codec (Curator's default image ships
    NVDEC-only h264/hevc/av1 decoders).
...
# MP4 sample-description FOURCCs for codecs that the default Curator image
# can only decode via NVDEC. Used by the heuristic header sniff below.
        ...
        try:
            ...
            if any(signal in stderr for signal in _CODEC_OPEN_FAILURE_SIGNALS):
                ...
                msg = (
                    ...
                    "The container's ffprobe ships NVDEC-only h264/hevc/av1 decoders, "
