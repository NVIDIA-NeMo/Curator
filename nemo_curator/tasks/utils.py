...
class TaskPerfUtils:
    ...
    @staticmethod
    def get_unique_stage_stat(
        tasks: list[Task] | WorkflowRunResult | Mapping[str, list[Task]] | None,
        stage_name: str,
        stat: str,
        pipeline_name: str | None = None,
    ) -> float:
        """Get a stat for exact stage matches, counting each perf record once.

        Some fan-out stages attach the same StagePerfStats object to multiple downstream tasks.
        This helper deduplicates those shared perf records by object identity before summing.

        Args:
            tasks: A list of Task objects, a WorkflowRunResult, or a mapping of pipeline_name -> list[Task].
            stage_name: Match stages with this exact name.
            stat: The stat to extract using StagePerfStats.items(), e.g. "custom.num_rows".
            pipeline_name: Optional pipeline name to limit the aggregation to.
        ...
        if pipeline_name is not None:
            pipeline_task_map = {pipeline_name: pipeline_task_map.get(pipeline_name, [])}

        total = 0.0
        seen_perf_ids: set[int] = set()
        for pipeline_tasks in pipeline_task_map.values():
            for task in pipeline_tasks or []:
