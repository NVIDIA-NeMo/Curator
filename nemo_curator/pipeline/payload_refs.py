...
@runtime_checkable
class PayloadEnvelope(Protocol):
    """Payload-neutral container contract preserved by lifecycle helpers."""

    @property
    def items(self) -> Sequence[Any]: ...

    def with_items(self, items: Sequence[Any]) -> Self: ...
...
class PayloadRef:
    """Serializable handle to a backend-managed, payload-neutral value."""
...
def iter_payload_refs(value: Any) -> list[PayloadRef]:
    ...
    if isinstance(value, PayloadRef):
        refs.append(value)
    elif isinstance(value, PayloadEnvelope):
        ...
        if isinstance(value, Task):
            refs.extend(iter_payload_refs(value.data))
    return refs


def task_payload_refs(task: Task) -> list[PayloadRef]:
    """Return recursively nested refs only for defensive cleanup paths."""
    return iter_payload_refs(task)


def map_payload_items(value: Any, transform: Callable[[Any], Any]) -> Any:
    """Transform leaf values while preserving generic ``items`` envelopes."""
    if isinstance(value, PayloadEnvelope):
        return value.with_items([map_payload_items(item, transform) for item in value.items])
    return transform(value)


def iter_payload_items(value: Any) -> list[Any]:
    """Return leaves from arbitrarily nested structural payload envelopes."""
    if isinstance(value, PayloadEnvelope):
        leaves: list[Any] = []
        for item in value.items:
            leaves.extend(iter_payload_items(item))
        return leaves
    return [value]
