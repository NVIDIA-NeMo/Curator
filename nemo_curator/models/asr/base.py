...
class ASRAdapter(Protocol):
    ...
    * ``audio_seconds`` (``float``): estimated item duration for metrics or
      adapter-side policy decisions.
    * ``chunk_idx`` / ``chunk_count`` (``int | None``): chunk position metadata
      when the stage split a long parent row. Adapters may ignore any metadata
    ...
    Optional methods:
        ``estimate_item_cost(item) -> float | None`` may be implemented by an
        adapter to provide a scheduler cost better than raw duration, such as
        estimated encoder tokens or approximate VRAM units. ``ASRStage`` falls
        back to duration when the method is absent or returns ``None``.
    """
